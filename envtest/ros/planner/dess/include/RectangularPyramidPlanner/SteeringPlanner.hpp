    ...
    int GetNextCandidateTrajectory(
      ...
      if (_sampling_mode == 2)
      {
        ...
        * (pixel_depth - DEPTH_LOWER_BOUND) + DEPTH_LOWER_BOUND : gen_depth;
        ...
        // For the benchmark to be fair, we will have 
        // constant planning velocity in the depth direction (Z direction)
        nextTraj.Generate(posf.GetNorm2());
      }
 ...
 private:
  ...
  class ExplorationCost {
   public:
    ...
    double GetCost(
        RapidQuadrocopterTrajectoryGenerator::SteeringTrajectoryGenerator& traj) {
