...
bool SteeringPlanner::FindLowestCostTrajectoryRandomCandidates(
    ...
    double (*costFunctionWrapper)(
        void* costFunctionDefinitionObject,
        RapidQuadrocopterTrajectoryGenerator::SteeringTrajectoryGenerator&)) {
...
bool SteeringPlanner::FindLowestCostTrajectory(
  ...
  SteeringTrajectoryGenerator candidateTraj(pos0, vel0, acc0, grav);
  while (true) {
    ...
    if (cost < bestCost) {
      ...
      SteeringTrajectoryGenerator::InputFeasibilityResult res = candidateTraj
      ...
      if (res == SteeringTrajectoryGenerator::InputFeasible) {
        ...
        if (isCollisionFree) {
          ...
          bestCost = cost;
          trajectory = SteeringTrajectoryGenerator(candidateTraj);
        }
      }
    }
  }
...
bool SteeringPlanner::IsCollisionFree(Trajectory trajectory) {
...
std::vector<MonotonicTrajectory> SteeringPlanner::GetMonotonicSections(
...
bool SteeringPlanner::FindDeepestCollisionTime(MonotonicTrajectory monoTraj,
                                                 Pyramid pyramid,
...
bool SteeringPlanner::IsCollisionFreeGroundTruth(Trajectory trajectory) {
